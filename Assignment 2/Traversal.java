...
public class Traversal {
    ...
    /**
     * TraveNode class for Traversal The only difference from Node class is it
     * contains an extra attribute: step this.step represents the traversing step in
     * this.next() method
     */
    ...
    /**
     * Inspired by the Python generator, this method return the next node in the
     * binary tree each time it is invoked, instead of traverse the whole tree each
     * time. It stores the path of traversal, and restore the path direcly to get
     * next node. Return null if no more node can be traversed
     * 
    ...
    private TravNode next(TravNode travNode) {
        ...
        // Steps are from 1 to 3, according to traversal mode, actions are made in
        // different order
